...
async fn get_mempool_info_inner() {
    ...
    let zcashd_mi = dbg!(zcashd_subscriber.get_mempool_info().await.unwrap());
    let zaino_mi = dbg!(zaino_subscriber.get_mempool_info().await.unwrap());

    assert_eq!(zcashd_mi, zaino_mi);
